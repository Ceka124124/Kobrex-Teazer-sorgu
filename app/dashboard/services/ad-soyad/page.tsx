...
export default function AdSoyadSorguPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/services/ad-soyad?${params}`)
        ...
        <Card className="bg-slate-800 border-slate-700 mb-6">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-sm text-slate-300 mb-2 block">İl (Opsiyonel)</label>
                <Input
                  type="text"
                  placeholder="İl girin"
                  value={il}
                  onChange={(e) => setIl(e.target.value.toUpperCase())}
                  className="bg-slate-700 border-slate-600 text-white placeholder:text-slate-400"
                />
              </div>

              <div>
                <label className="text-sm text-slate-300 mb-2 block">İlçe (Opsiyonel)</label>
                <Input
                  type="text"
